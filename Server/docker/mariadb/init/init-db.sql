drop
database if exists `peluqueriaDAM`;
create
database `peluqueriaDAM`;
use
`peluqueriaDAM`;
...
alter table appointment
    add constraint FK9l9i8coynlm7vocln34x1ingn foreign key (id_user) references users (id);
alter table login
    add constraint FKh5dnpi129vsjqrc1b9qc2xwck foreign key (id_user) references users (id);
